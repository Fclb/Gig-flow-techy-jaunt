...
app.get("/find-providers-by-name", async (req, res) => {
    const { name } = req.query;
    try {
        const providers = await Provider.find({ fullName: { $regex: name, $options: "i" } });
        return res.status(200).json({ providers });
    } catch (error) {
        console.error("Error fetching Providers:", error);
        return res.status(500).json({ message: "Internal server error" });
...
app.get("/providers", async (req, res) => {
    ...
    try {
        ...
        if (skill) {
            filter.skillCategory = { $regex: skill, $options: "i" }; // case-insensitive
        }
...
app.patch("/update-provider/:id", async (req, res) => {
    const { id } = req.params;
    const { fullName, email, phoneNumber, skillCategory, gender, address, isVerified } = req.body;
    try {
        const updatedProvider = await Provider.findByIdAndUpdate(
